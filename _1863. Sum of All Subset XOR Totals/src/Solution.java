...
public class Solution {
    ...
    public int subsetXORSum(int[] nums, int index, int result) {
        if (index == nums.length) {
            return result;
        }
        int complex = subsetXORSum(nums, index + 1, result ^ nums[index]);
        int simple = subsetXORSum(nums, index + 1, result);
